...
var connectionString = "Host=127.0.0.1;Username=kamlesh;Database=pizzahub;Port=5432;";
...
app.MapGet("/categories", async (HttpContext context) =>
{
    ...
    using (var conn = new NpgsqlConnection(connectionString))
    {
        ...
        using (var cmd = new NpgsqlCommand("SELECT CategoryID, Name, Description, CreatedAt FROM categories ORDER BY CategoryID ASC", conn))
...
async Task InitializeDatabaseAsync(string connStr)
{
    using (var conn = new NpgsqlConnection(connStr))
    {
        ...
        using (var checkCmd = new NpgsqlCommand("SELECT COUNT(*) FROM categories", conn))
        {
...
async Task SeedDatabaseAsync(NpgsqlConnection conn)
{
    string? seedDir = null;
    var pathsToTry = new[] {
    ...
    if (File.Exists(categoriesPath))
    {
        ...
        if (categories != null)
        {
            using (var tx = conn.BeginTransaction())
            {
                ...
                {
                    using (var cmd = new NpgsqlCommand("INSERT INTO categories (CategoryID, Name, Description, CreatedAt) VALUES (@id, @name, @desc, @created)", conn, tx))
                    {
